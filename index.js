...
app.get('/user',  (req, res) => {
  ...
  db.all('SELECT * FROM advertisement WHERE username = ? ', [q], (err, rows) => {
    if (err) {
      ...
      console.log(rows);
      res.render("temp.ejs", { flashMessage: req.flash('success') ,user: req.session.user ,roll:req.session.roll ,name:req.session.name ,Data:rows })
...
app.get('/signin',  (req, res) => {
  ...
  // Render homepage with message (if present)
  // res.sendFile(__dirname+'/views/login.ejs', { flashMessage: req.flash('success') });
  res.render("login2.ejs", { flashMessage: req.flash('success') ,user: req.session.user , name:req.session.name ,roll:req.session.roll })
});
app.get('/signup',  (req, res) => {
  ...
  // Render homepage with message (if present)
  // res.sendFile(__dirname+'/views/login.ejs', { flashMessage: req.flash('success') });
  res.render("signup.ejs", { flashMessage: req.flash('success'),user: req.session.user , name:req.session.name ,roll:req.session.roll })
});
